...
## Phase 6: AI 품질 고도화 (지속)
> RAG 검색 정확도 및 LLM 프롬프트 품질을 데이터 기반으로 개선.
> 운영 데이터 축적 후 진행 (Phase 3 완료 + 실 트래픽 수집 이후).

### 6-A: RAG 품질 개선
- [ ] 6.1 유사도 임계값 컷오프 도입 (`backend/035`) ← 3.4
  > VectorSearchService에 최소 유사도 필터 추가 — 낮은 점수 청크 제거로 LLM 노이즈 감소
- [ ] 6.2 청크 세분화 전략 개선 (`backend/036`) ← 3.2
  > 조문→항 단위 분할, 청크 오버랩(sliding window) 도입으로 검색 정밀도 향상
- [ ] 6.3 임베딩 텍스트 전처리 개선 (`backend/037`) ← 3.2
  > 법령 특수문자·약어 정규화, 동의어 확장 → 코사인 유사도 정확도 상승
- [ ] 6.4 RAG 검색 평가 파이프라인 (`backend/038`) ← 6.1,6.2,6.3
  > 검색 결과 Precision@K / Recall@K 측정 도구, 골든셋 기반 자동 평가

### 6-B: 프롬프트 검증·개선
- [ ] 6.5 프롬프트 A/B 테스트 프레임워크 (`backend/039`) ← 3.6
  > 프롬프트 버전 관리(DB) + 응답 품질 지표 수집(만족도, 정확도)
- [ ] 6.6 Few-shot 예시 주입 개선 (`backend/040`) ← 6.5
  > 채널별·법령별 대표 위반 사례를 시스템 프롬프트에 동적 주입
- [ ] 6.7 규칙 변환 프롬프트 품질 평가 (`backend/041`) ← 3.11,6.5
  > LawRuleConversionService 생성 규칙 품질 측정 — 운영자 승인률 추적·피드백 루프

### 6-C: 기존 추가 기능
- [ ] 6.8 백오피스 `frontend/010`
- [ ] 6.9 산업별 규칙 `backend/030` ← 1.3
- [ ] 6.10 멀티유저 API `backend/031` ← 4.2
- [ ] 6.11 멀티유저 화면 `frontend/011` ← 6.10
- [ ] 6.12 Slack 봇 `backend/032`
- [ ] 6.13 커스텀 규칙 API `backend/033` ← 1.3
- [ ] 6.14 커스텀 규칙 화면 `frontend/012` ← 6.13
